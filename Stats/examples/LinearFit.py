...
def main():
    filename = str(input("File : "))          # Get filename
    data = f.readCSV(filename)                # Read in data to array

    x = data[0]                               # set x/y to first two cols
    y = data[1]
    ...
    #       Label the graph
    plt.title("Linear Fit a: {0:8.4g} b: {1:8.4g}".format(popt[0],popt[1]))
    plt.xlabel("x value")
    plt.ylabel("y value")
    
    #       Lower plot of residuals
    plt.subplot(2,1,2)
    plt.errorbar(x,linear(x,*popt) - y,xerr = 0.0, yerr = yErr,fmt = "rx")
    plt.xlabel("x value")
